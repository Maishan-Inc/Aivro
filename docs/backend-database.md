...
画布助手调用后台配置的默认文本模型。普通问答仍按模型算力点配置扣费；画布 Agent 规划接口会在调用模型前按估算 token 预扣，成功后读取模型返回的 token usage，按每 1000 token 乘以该模型算力点配置向上取整多退少补，并把本轮返回的结构化画布操作和用量保存到消息中。过期历史会在读取助手列表时软删除。
