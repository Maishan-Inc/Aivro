...
import { ApiOutlined, AuditOutlined, DatabaseOutlined, FileTextOutlined, GlobalOutlined, HomeOutlined, LogoutOutlined, MailOutlined, MenuOutlined, PictureOutlined, ShareAltOutlined, ShoppingOutlined, SettingOutlined, TransactionOutlined, UserOutlined } from "@ant-design/icons";
import { Button, Drawer, Dropdown, Flex, Layout, Menu, Typography, theme } from "antd";
    ...
    return (
        <Layout hasSider style={{ height: "100vh", overflow: "hidden", background: antToken.colorBgLayout }}>
            <Layout.Sider className="hidden lg:block" width={adminLayoutStyle.siderWidth} style={{ height: "100vh", overflow: "hidden", background: antToken.colorBgContainer, borderRight: `1px solid ${antToken.colorBorder}` }}>
                <Flex align="center" gap={12} style={{ height: adminLayoutStyle.brandHeight, padding: "0 28px", borderBottom: `1px solid ${antToken.colorBorderSecondary}` }}>
            ...
            <Layout style={{ background: antToken.colorBgLayout }}>
                <Layout.Header
                    className="gap-3 px-3 lg:px-6"
                    style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: adminLayoutStyle.headerHeight, background: antToken.colorBgContainer, borderBottom: `1px solid ${antToken.colorBorder}` }}
                >
                    <Flex align="center" gap={8} style={{ minWidth: 0 }}>
                        <Button className="lg:hidden" type="text" icon={<MenuOutlined />} onClick={() => setMenuOpen(true)} aria-label="打开后台菜单" />
                ...
                <Layout.Content style={{ minHeight: 0, overflow: "auto" }}>
                    {children}
                    <div className="px-3 py-3 lg:px-6" style={{ textAlign: "center", color: antToken.colorTextTertiary, fontSize: 12 }}>Copyright © 2026 Maishan Inc. All rights reserved Aivro</div>
                </Layout.Content>
            </Layout>
            <Drawer title="管理后台" placement="left" width="min(320px, 88vw)" open={menuOpen} onClose={() => setMenuOpen(false)} styles={{ body: { padding: 0 } }}>
                <Flex vertical style={{ minHeight: "100%" }}>
                    ...
                    <Menu mode="inline" selectedKeys={[activeKey]} style={{ ...adminLayoutStyle.menu, flex: 1 }} items={menuItems} onClick={() => setMenuOpen(false)} />
                    <Flex vertical gap={8} style={{ padding: 12, borderTop: `1px solid ${antToken.colorBorder}`, background: antToken.colorBgContainer }}>
                        <Button block icon={<HomeOutlined />} href="/" target="_blank" rel="noreferrer">
                            前往站点首页
                        </Button>
                        <Button block icon={<LogoutOutlined />} onClick={logout}>
