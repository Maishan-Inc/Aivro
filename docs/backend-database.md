...
MetaMask 登录服务端签名挑战表。nonce 与钱包地址绑定，短时有效且只能使用一次。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | string | 主键 |
| `wallet_address` | string | 小写钱包地址 |
| `nonce` | string | 服务端生成的一次性随机值，唯一索引 |
| `message` | text | 服务端生成并要求钱包签名的完整消息 |
| `expires_at` | string | 过期时间 |
| `used_at` | string | 使用时间，未使用为空 |
| `created_at` | string | 创建时间 |
...
提示词分组表。用于保存内置提示词分组的展示状态和远程来源信息。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `category` | string | 主键，分组标识 |
| `name` | string | 分组名称 |
| `description` | string | 分组说明 |
| `github_url` | string | 远程来源地址 |
| `remote` | bool | 是否为远程同步分组 |
| `enabled` | bool | 是否在首页、提示词库和后台提示词列表展示 |
| `updated_at` | string | 更新时间 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `modelId` | string | 渠道模型隐藏 ID，用于计费匹配和审计；由后端按新增顺序分配，删除后不复用旧 ID |
| `model` | string | 当前前台模型名；展示名称变更时随渠道模型同步更新 |
| `credits` | number | 计费值；按次计费时表示每次调用点数，Token 计费时表示每 1000 token 点数 |
| `billingType` | string | 计费方式：`fixed` 按次计费，`token` 按 Token 计费 |

`modelCosts` 只跟随 `availableModels` 中开放给前台的模型生成，内部优先按 `modelId` 匹配计费配置，不再依赖模型名称作为唯一计费标识。
...
| `captcha` | object | 人机验证公开配置，包含启用状态、服务提供方和前端 Site Key |
| `turnstileSiteKey` | string | 旧版 Cloudflare Turnstile 前端站点 Key 兼容字段，新逻辑优先使用 `captcha` |
...
| `modelIdSeq` | number | 渠道模型隐藏 ID 的当前序列值，用于保证删除后新增模型不复用旧 ID |
| `runtime` | object | 运行期配置，后台值优先，环境变量兜底 |
| `promptSync` | object | GitHub 远程提示词定时同步配置 |
| `aiQueue` | object | AI 队列与限流配置 |
| `canvasAssist` | object | 画布助手配置 |
| `captcha` | object | 人机验证私有配置，支持 `turnstile` 和 `hcaptcha` 两种提供方 |
| `turnstile` | object | 旧版 Cloudflare Turnstile 私有配置兼容字段，读取时会映射到 `captcha.turnstile` |
| `auth` | object | 私有登录配置 |
...
| `name`   | string   | 渠道名称     |
| `color` | string | 渠道颜色，后台模型标签按该颜色展示 |
| `baseUrl` | string  | 渠道接口地址   |
| `apiKey` | string   | 渠道密钥     |
| `models` | string[] | 兼容字段，保存当前渠道映射后的前台模型名列表 |
| `modelMappings` | object[] | 渠道模型映射列表 |
| `weight` | number   | 渠道权重，同一前台模型命中多个渠道时按权重从高到低尝试 |
| `enabled` | bool    | 是否启用     |
| `remark` | string   | 备注       |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | string | 渠道内模型隐藏 ID，用于计费和审计；新增模型由后端按顺序分配，删除后后续新增不补用旧 ID |
| `name` | string | 前台模型名，用于用户选择和公开模型列表 |
| `upstreamName` | string | 上游模型名，实际请求渠道时写入 `model` 字段 |
| `capability` | string | 模型能力：`image`、`text`、`video`、`model3d` |

`runtime` 字段：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `appOrigin` | string | 站点可信外部访问地址，用于 OAuth 回调、支付回跳、KYC 回调和分享链接；留空时沿用 `APP_ORIGIN` |
| `allowedOrigins` | string | 允许从请求 Host 推断来源的域名列表，多个用英文逗号分隔；留空时沿用 `ALLOWED_ORIGINS` |
| `jwtExpireHours` | number | 新签发 JWT 和登录 Cookie 的有效期小时数；不保存 `JWT_SECRET` |

`PORT`、`STORAGE_DRIVER`、`DATABASE_DSN`、`LOCAL_FILE_DIR`、`JWT_SECRET` 和首次默认管理员账号密码仍是启动期环境变量，不保存到后台配置。
...
后端请求模型时，先按前台模型名筛选启用且包含该模型映射的渠道，并把请求里的 `model` 改写为对应 `upstreamName`。同一前台模型命中多个渠道时先使用权重最高的渠道；请求失败后按更低权重渠道继续尝试，最后一个可用渠道会再重试一次，全部失败后返回上游错误状态码。
...
画布助手调用后台配置的默认对话模型，也可由前端在对话模型范围内指定本轮模型。普通问答仍按模型算力点配置扣费；画布 Agent 规划接口会在调用模型前按估算 token 预扣，成功后读取模型返回的 token usage，按每 1000 token 乘以该模型算力点配置向上取整多退少补，并把本轮返回的结构化画布操作和用量保存到消息中。过期历史会在读取助手列表时软删除。
...
| `sort` | number | 排序 |
| `translations` | json | 多语言覆盖，可覆盖名称、描述、功能点、价格、币种和额度 |
| `created_at` | string | 创建时间 |
| `updated_at` | string | 更新时间 |
...
用户请求日志和算力点变更流水表。当前记录后台手动调整、模型调用预扣、0 计费模型请求和模型调用失败返还。
